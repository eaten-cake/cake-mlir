...
private:
  ...
  llvm::LogicalResult mlirGen(StructAST &str) {
    ...
    structMap.try_emplace(str.getName(), ToyStructType::get(builder.getContext(), elementTypes), &str);
    return mlir::success();
  }
  ...
  getConstantAttr(StructLiteralExprAST &lit) {
    ...
    mlir::Type dataType = ToyStructType::get(builder.getContext(), typeElements);
